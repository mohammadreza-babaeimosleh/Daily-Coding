"""
test.py — Benchmark & correctness tester for LeetCode 26 · Remove Duplicates from Sorted Array
...
def reference(nums: list) -> tuple:
    """Returns (k, unique_elements) without modifying input."""
...
def generate_test_cases(count: int) -> list:
    fixed = [
        ...
        {"nums": list(range(-100, 101))},  # all unique, max stress
    ]
    ...
    while len(cases) < count:
        ...
        nums   = sorted(rng.randint(-100, 100) for _ in range(length))
...
def run_tests(num_tests: int = 100) -> None:
    print("=" * 60)
    print("  LeetCode 26 · Remove Duplicates from Sorted Array — Test Suite")
    print("=" * 60)
    ...
    for idx, case in enumerate(cases):
        ...
        try:
            ...
            failures.append({
                ...
                "nums_in": case["nums"],
                "got":     f"EXCEPTION: {exc}",
                "expected": f"k={case['expected_k']}, first-k={case['expected']}",
            })
