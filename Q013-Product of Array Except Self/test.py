"""
test.py — Benchmark & correctness tester for Product of Array Except Self
Runs 100 test cases against main.py's Solution.productExceptSelf() and reports:
  - Pass rate
...
def generate_test_cases(count: int) -> list:
    fixed = [
        ...
        {"nums": [0, 0, 1, 2]},
        {"nums": [-1, -1, -1, -1]},
        {"nums": [30, -30] * 500},    # 1000-element stress case
    ]
    cases = []
    for f in fixed:
...
def run_tests(num_tests: int = 100) -> None:
    print("=" * 60)
    print("  Product of Array Except Self — Test Suite")
    print("=" * 60)
    ...
    for idx, case in enumerate(cases):
        ...
        try:
            ...
            elapsed = time.perf_counter_ns() - start
            failures.append({"index": idx+1, "nums_in": case["nums"], "got": f"EXCEPTION: {exc}", "expected": case["expected"]})
        ...
        if result == case["expected"]:
            ...
            failures.append({"index": idx+1, "nums_in": case["nums"], "got": result, "expected": case["expected"]})
