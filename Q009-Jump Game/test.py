"""
test.py — Benchmark & correctness tester for LeetCode 55 · Jump Game
Runs 100 test cases against main.py's Solution.canJump() and reports:
  - Pass rate
...
def generate_test_cases(count: int) -> list:
    fixed = [
        ...
        {"nums": [0]},                       # single element → true
        {"nums": [1]},                       # single element → true
        {"nums": [0, 1]},                    # stuck at index 0 → false
        {"nums": [1, 0]},                    # just makes it → true
        {"nums": [2, 0, 0]},                 # skip over zeros → true
        {"nums": [1, 0, 0]},                 # can't reach end → false
        {"nums": [100_000] + [0] * 9999},    # max jump from start → true
        {"nums": [0] * 9999 + [1]},          # blocked immediately → false
    ]
    ...
    while len(cases) < count:
        ...
        nums = [rng.randint(0, 100_000) for _ in range(n)]
...
def run_tests(num_tests: int = 100) -> None:
    print("=" * 60)
    print("  LeetCode 55 · Jump Game — Test Suite")
    print("=" * 60)
